...
async function test() {
  ...
  17606609
  await getNFTs(17606609, 'Testing all NFT types in the latest block with 100 block confirmations', 'erc-1155');
  ...
  async function getNFTs(blockNumber: any, message: string, type: string) {
    ...
    if (type === 'erc-1155') {
      ...
      const result = await onChainNFT.getEvmNFTs({ blockNumber: blockNumber });
      console.log(JSON.stringify(result));
      console.log('found ' + result.length + ' NFTs in block:' + blockNumber);
    }
  }
